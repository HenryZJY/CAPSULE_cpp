//
// Created by Junyan Zhang on 5/14/20.
//
...
//                std::cout << "LSH insert single int, for-loop " << table << std::endl;
...
void LSH::view() {
        for (size_t t = 0; t < L; t++) {
                printf("LSH Table %lu\n", t);
                for (size_t r = SHOWRANGE; r < SHOWRANGE+20; r++) {
                        reservoirs[t][r].view();
                }
                printf("\n");
        }
